...
export default function Map(props: any) {
  ...
  const [shidianPoint, setShidianPoint] = useState<any>(null);
  ...
  const setPoint = (viewer: Cesium.Viewer, position: number[]) => {
    if (!shidianPoint) {
      ...
      console.warn('[shidianPoint]内容已存在!');
    }
  };
      ...
      <div className={styles.btnWrap} style={{ top: 8 }}>
        <div className={styles.btnItem} style={{ marginBottom: 8 }}>
          <Button
            type="primary"
            onClick={() => {
              setPoint(viewerRef.current, [109.502991, 29.68718, 200]);
              setFlyToBoundingSphereOrigin(
                viewerRef.current,
                [109.502991, 29.68718, 200],
              );
            }}
          >
            十点珠
